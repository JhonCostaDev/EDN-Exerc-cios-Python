...
#read_files_names = lambda dictionary: sum(dictionary.values(), [])
def read_files_names(dictionary):
...
def main():
    ...
    class_folders = [f'{item}/atividades' for item in os.listdir() if item.startswith('aula')]
    class_folders.sort()
    ...
    while True:
        ...
        dict_files = {x: os.listdir(class_folders[x]) for x in range(len(class_folders))}
    
        dict_all_files = read_files_names(dict_files) 
        option = menu(dict_all_files) 
        print(option)

        os.system(switch(option, class_folders, dict_all_files))
